...
- execute retries may preserve only artifacts from earlier successful prompts in the queue: review inputs for all retries, triage/plan only after prompt 1, and never a prior attempt's `round-status.json` or `round-summary.md`.
