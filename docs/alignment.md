...
| R-21 | Do not push in intermediate rounds; allow delivery-stage pushes only after rounds are complete, including bounded fix-and-retry pushes when needed to reach a merge-ready result. | `docs/spec.md`, `docs/architecture.md`, `docs/decisions.md` |
...
| R-35 | Execute stage runs an ordered two-prompt queue sequentially in one fresh Codex chat context, with review artifact paths and a compact manifest provided as prompt context. | `docs/spec.md`, `docs/architecture.md`, `prompts/execute/queue.txt`, `prompts/execute/01-triage-plan.md`, `prompts/execute/02-implement-verify-finalize.md` |
...
| R-47 | Execute prompt wiring should use review-stage terminology (`REVIEW_*` placeholders) while retaining compatibility for legacy placeholder names where still needed. | `internal/deepreview/orchestrator.go`, `docs/decisions.md` |
...
| R-49 | Delivery runs in one fresh Codex context that owns branch/PR readiness, PR text upkeep, push/PR actions, remote-check waiting, and high-confidence fix-and-retry follow-through until mergeable or clearly blocked. | `internal/deepreview/orchestrator.go`, `prompts/delivery/01-deliver.md`, `docs/spec.md`, `docs/architecture.md`, `docs/decisions.md` |
