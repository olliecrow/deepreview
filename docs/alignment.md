...
| R-67 | Every completed run must publish canonical run-health artifacts that summarize artifact coverage and observational stderr noise, and terminal summary recovery must backfill them whenever a root `final-summary.md` already exists without them. | `docs/spec.md`, `docs/architecture.md`, `docs/decisions.md`, `internal/deepreview/run_health.go`, `internal/deepreview/orchestrator.go`, `internal/deepreview/cli.go`, `internal/deepreview/orchestrator_test.go`, `internal/deepreview/cli_test.go` |
