...
| R-50 | All Codex prompt calls (new/resume, including post-delivery PR enhancement) must be hard-pinned to `gpt-5.4` with `model_reasoning_effort=high`. | `internal/deepreview/codex.go`, `internal/deepreview/orchestrator.go`, `internal/deepreview/cli.go`, `docs/spec.md`, `docs/decisions.md` |
...
| R-60 | All Codex prompt executions must inherit writable worktree-local temp/cache defaults so verification tools stay inside deepreview-managed sandbox paths instead of host-global caches. | `internal/deepreview/codex.go`, `internal/deepreview/process.go`, `internal/deepreview/codex_test.go`, `internal/deepreview/process_test.go`, `internal/deepreview/integration_test.go`, `docs/spec.md`, `docs/architecture.md`, `docs/decisions.md` |
