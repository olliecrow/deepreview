...
| R-43 | When source branch is inferred from local context, deepreview requires no tracked local changes and verifies exact local/upstream sync against fresh upstream state before run start. | `README.md`, `docs/spec.md`, `docs/architecture.md`, `internal/deepreview/local_context.go`, `internal/deepreview/local_context_test.go` |
...
| R-58 | Before any outward-facing delivery action, privacy handling is a bounded Codex-led remediation loop (max 3 attempts) with mandatory final rescans; unresolved findings block PR delivery, yolo pushes, and incomplete-draft publication. | `internal/deepreview/orchestrator.go`, `prompts/delivery/privacy-fix.md`, `internal/deepreview/integration_test.go`, `docs/spec.md`, `docs/architecture.md`, `docs/decisions.md`, `README.md` |
