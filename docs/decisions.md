...
This log records cross-cutting product rationale that is not clearer in code, tests, the runtime contract, or the architecture.
...
References:
`docs/spec.md`, `internal/deepreview/orchestrator.go`, `internal/deepreview/privacy_test.go`, `.pre-commit-config.yaml`
...
Decision:
Use change-driven round control: `continue` always continues, repository changes receive another independent review, and an unchanged `stop` ends the loop.
Context:
deepreview needs bounded iteration without repeating a clean review or delivering changes that have not received an independent follow-up review.
Rationale:
Combining the explicit execute decision with actual repository changes gives each additional round a concrete reason.
Trade-offs:
Runs that keep changing the candidate branch require more rounds and can exhaust `--max-rounds`; clean runs stop after one round.
Enforcement:
