...
20. If you use Go tooling, the inherited environment already points temp/cache paths at writable run-scoped directories outside the repo worktree; use that environment exactly as inherited and do not override `GOCACHE`, `GOMODCACHE`, `GOTMPDIR`, `TMPDIR`, `TMP`, or `TEMP`.
